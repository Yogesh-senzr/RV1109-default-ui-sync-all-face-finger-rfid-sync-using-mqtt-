...
#define DEFAULT_VIRTUALKEYBOARD_HEIGHT (280)

frmInput *frmInput::_instance = 0;
frmInput::frmInput(QWidget *parent) :
    QWidget(parent), ui(new Ui::frmInput)
{
    ui->setupUi(this);
    this->InitProperty();
    this->InitForm();
...
void frmInput::InitForm()
{
    ...
    frmWidth = deskWidth;
    frmHeight = DEFAULT_VIRTUALKEYBOARD_HEIGHT*deskHeight/1280;
    //printf("%s %s[%d] deskWidth %d  deskHeight %d  frmWidth %d  frmHeight %d \n",
    //__FILE__, __FUNCTION__, __LINE__, deskWidth, deskHeight, frmWidth, frmHeight);

    QSqlDatabase DbConn;
    DbConn = QSqlDatabase::addDatabase("QSQLITE");
    DbConn.setDatabaseName("/isc/pinyin.db");
    DbConn.open();
...
void frmInput::focusChanged(QWidget *oldWidget, QWidget *nowWidget)
{
    ...
    if (nowWidget != 0 && !this->isAncestorOf(nowWidget))
    {
        ...
        if (nowWidget->inherits("QLineEdit"))
        {
            ...
            ShowPanel();
        } else if (nowWidget->inherits("LineEdit"))
        {
            currentLineEdit = (QLineEdit *) nowWidget;
            currentEditType = "QLineEdit";
            ShowPanel();
        ...
        {
            this->setGeometry(0, deskHeight - frmHeight - 80, deskWidth, frmHeight);
...
void frmInput::changeStyle(QString topColor, QString bottomColor, QString borderColor, QString textColor)
{
    QStringList qss;
    qss.append(
                QString("QWidget#frmInput{background:qlineargradient(spread:pad,x1:0,y1:0,x2:0,y2:1,stop:0 %1,stop:1 %2);}").arg(topColor).arg(
                    bottomColor));
    qss.append("QPushButton{padding:5px;border-radius:3px;}");
    qss.append(
                QString("QPushButton:hover{background:qlineargradient(spread:pad,x1:0,y1:0,x2:0,y2:1,stop:0 %1,stop:1 %2);}").arg(topColor).arg(
                    bottomColor));
    ...
    qss.append(
                QString(
                    "QLineEdit{border:1px solid %1;border-radius:5px;padding:2px;background:none;selection-background-color:%2;selection-color:%3;}").arg(
                    borderColor).arg(bottomColor).arg(topColor));
