...
{
    struct usb_host_context *context = (struct usb_host_context*)malloc(sizeof(struct usb_host_context));
    memset(context,0,sizeof(struct usb_host_context));
    if (!context) {
                  ...
                  void *client_data)
{
    ...
    printf("%s %s[%d] Created device discovery thread\n",__FILE__,__FUNCTION__,__LINE__);
    ...
    done = find_existing_devices(added_cb, client_data); //liwen
    if (discovery_done_cb)
...
//        	 printf("%s %s[%d] \n",__FILE__,__FUNCTION__,__LINE__);
            ...
            if (wd == context->wdd) {
                ...
                printf("%s %s[%d] %s subdirectory %s: index: %d\n",__FILE__,__FUNCTION__,__LINE__, (event->mask & IN_CREATE) ?
                        "new" : "gone", path, i);
                ...
                for (i = 1; (i < MAX_USBFS_WD_COUNT) && !done; i++) {
                    if (wd == context->wds[i]) {
                        ...
                        if (event->mask == IN_CREATE) {
                            printf("%s %s[%d] new device %s\n",__FILE__,__FUNCTION__,__LINE__, path);
                            done = context->cb_added(path, context->data);
                        } else if (event->mask == IN_DELETE) {
                            printf("%s %s[%d] gone device %s\n",__FILE__,__FUNCTION__,__LINE__, path);
...
//    printf("%s %s[%d] usb_device_open %s\n", __FILE__,__FUNCTION__,__LINE__,dev_name);
...
//        printf("%s %s[%d] usb_device_open open returned %d errno %d\n", __FILE__,__FUNCTION__,__LINE__,fd, errno);
        ...
        if (fd < 0)
            return NULL;
        writeable = 0;
//        printf("%s %s[%d] [ usb open read-only %s fd = %d]\n", __FILE__,__FUNCTION__,__LINE__,dev_name, fd);
    }
...
{
    struct usb_device *device = (struct usb_device*)malloc(sizeof(struct usb_device));
    memset(device,0,sizeof(struct usb_device));
    int length;

//    printf("%s %s[%d] usb_device_new %s fd: %d\n", __FILE__,__FUNCTION__,__LINE__,dev_name, fd);
    ...
    if (length < 0)
    {
        printf("%s %s[%d] usb_device_new read returned %d errno %d , %s \n", __FILE__,__FUNCTION__,__LINE__,length, errno,strerror(errno));
    	goto failed;
    }
...
//    printf("%s %s[%d] usb_device_reopen_writeable failed errno %d\n",__FILE__,__FUNCTION__,__LINE__, errno);
    return 0;
}
...
char* usb_device_get_name_from_unique_id(int id)
{
    int bus = id / 1000;
    int dev = id % 1000;
    char* result = (char *)malloc(strlen(USB_FS_ID_FORMAT));
    memset(result,0,strlen(USB_FS_ID_FORMAT));
...
{
//    printf("%s %s[%d] dev_name :%s interface :%d \n",__FILE__,__FUNCTION__,__LINE__,device->dev_name,interface);
...
//    printf("%s %s[%d]  endpoint :%d length :%d ",__FILE__,__FUNCTION__,__LINE__,endpoint,length);
...
{
    struct usbdevfs_urb *urb = (struct usbdevfs_urb*)malloc(sizeof(struct usbdevfs_urb));
    ...
    if ((ep_desc->bmAttributes & USB_ENDPOINT_XFERTYPE_MASK) == USB_ENDPOINT_XFER_BULK)
        ...
        printf("%s %s[%d] Unsupported endpoint type %d", __FILE__,__FUNCTION__,__LINE__,ep_desc->bmAttributes & USB_ENDPOINT_XFERTYPE_MASK);
        free(urb);
        return NULL;
    }
    urb->endpoint = ep_desc->bEndpointAddress;

    struct usb_request *req = (struct usb_request*)malloc(sizeof(struct usb_request));
...
struct usb_request *usb_request_wait(struct usb_device *dev)
{
    ...
    while (1) {
        ...
        printf("%s %s[%d] USBDEVFS_REAPURB returned %d\n", __FILE__,__FUNCTION__,__LINE__,res);
        if (res < 0) {
            if(errno == EINTR) {
                continue;
            }
            printf("%s %s[%d] [ reap urb - error ]\n",__FILE__,__FUNCTION__,__LINE__);
            return NULL;
        } else {
            printf("%s %s[%d] [ urb @%p status = %d, actual = %d ]\n",__FILE__,__FUNCTION__,__LINE__,
